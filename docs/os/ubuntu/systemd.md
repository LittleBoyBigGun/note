...
## Systemd 架构
![[Pasted image 20230414122446.png]]
...
![[Pasted image 20230414123101.png]]
...
-   TimeoutStartSec：启动服务时等待的秒数，这一配置对于使用 [Docker](https://cloud.tencent.com/product/tke?from=20065&from_column=20065) [容器](https://cloud.tencent.com/product/tke?from=20065&from_column=20065)而言显得尤为重要，因其第一次运行时可能需要下载镜像，严重延时会容易被 Systemd 误判为启动失败杀死。通常，对于这种服务，将此值指定为 0，从而关闭超时检测
...
Systemd 通过其标准[日志服务](https://cloud.tencent.com/product/cls?from=20065&from_column=20065) Journald 提供的配套程序 journalctl 将其管理的所有后台进程打印到 std:out（即控制台）的输出重定向到了日志文件。
